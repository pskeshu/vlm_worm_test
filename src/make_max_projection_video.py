#!/usr/bin/env python3
"""
Create a max projection video from C. elegans embryo time series.
Reads 3D TIFF stacks, creates max projections, and generates an animated video with timestamps.
"""
...
def create_max_projection_video(
    embryo_dir='nih-ls/nih_diSPIM_deconv_1',
    output_video='embryo1_max_projection.mp4',
    fps=10,
    pixel_size_um=0.1625,
    scale_bar_um=10
):
    """
    Create max projection video from 3D TIFF time series.
    ...
    # Initialize video writer (color output for annotations)
    ...
    print(f"Total frames: {len(tiff_files)}")
    print(f"Video duration: {len(tiff_files)/fps:.1f} seconds")

if __name__ == '__main__':
    create_max_projection_video()
